/*
GENERATE FINAL SUDOKU FOR USER TO SOLVE
...
class Sudoku
{
    ...
    vector<vector<int>> state;  //State of board while deleting cells
    vector<vector<int>> solution;   //Store solution
    int n;
...
{
    //We will remove maximum 45 cells

    //Set board as solution
    solution=board;

    //Set initial state of all cells as 1(filled)
    for(int i=0 ; i<n ; i++)
        for(int j=0 ; j<n ; j++)
            state[i][j]=1;

    //State 0 is for empty cell
    //State -1 is for a cell that cannot be removed
    //int count=0;    //Removed cell count
    for(int i=1 ; i<45 ; i++)
    {
        ...
        //Check if cell is already deleted or cannot be deleted
        if(state[row][col]==1)
        {
            //Remove the cell
            state[row][col]=0;
            ...
            //Check if multiple solutions are possible
            //Fill current cell with numbers other than original number and run the solver
            for(int j=1 ; j<=9 ; j++)
            {
                if(solution[row][col]==j)   continue;
